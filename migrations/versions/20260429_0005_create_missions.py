...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(["mission_assignment_id"], ["mission_assignments.id"], name="fk_mission_participants_mission_assignment_id_mission_assignments"),
        sa.ForeignKeyConstraint(["doctor_id"], ["doctors.id"], name="fk_mission_participants_doctor_id_doctors"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["calendar_version_id"], ["calendar_versions.id"], name="fk_mission_candidate_rankings_calendar_version_id_calendar_versions"),
    )
    ...
    op.create_table(
        ...
        sa.Column("reasons", sa.JSON(), nullable=True),
        sa.Column("warnings", sa.JSON(), nullable=True),
        sa.ForeignKeyConstraint(["mission_candidate_ranking_id"], ["mission_candidate_rankings.id"], name="fk_mission_candidate_ranking_entries_mission_candidate_ranking_id_mission_candidate_rankings"),
        sa.ForeignKeyConstraint(["doctor_id"], ["doctors.id"], name="fk_mission_candidate_ranking_entries_doctor_id_doctors"),
    )
    op.create_index("ix_mission_candidate_ranking_entries_mission_candidate_ranking_id", "mission_candidate_ranking_entries", ["mission_candidate_ranking_id"])


def downgrade() -> None:
    op.drop_index("ix_mission_candidate_ranking_entries_mission_candidate_ranking_id", table_name="mission_candidate_ranking_entries")

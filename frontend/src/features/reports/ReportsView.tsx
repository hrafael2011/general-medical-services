...
export function ReportsView() {
  ...
  async function handleDownloadHistory() {
    ...
    try {
      ...
      addToast("success", "Historial descargado.");
      triggerBlobDownload(blob, `historial_medicos_${year}_${String(month).padStart(2, "0")}.xlsx`);
    } catch (err: unknown) {
      addToast("error", err instanceof Error ? err.message : "Error al descargar.");
    } finally {
  ...
  async function handleOperationalSummary() {
    ...
    try {
      ...
      addToast("error", err instanceof Error ? err.message : "Error al obtener resumen operacional.");
    } finally {
      setOpLoading(false);
    }
  }
  ...
  async function handleNotifSummary() {
    ...
    try {
      ...
      addToast("error", err instanceof Error ? err.message : "Error al obtener resumen de notificaciones.");
    } finally {

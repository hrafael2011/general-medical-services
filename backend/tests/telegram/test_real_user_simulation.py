...
def _seed_simulation_db(db_session):
    ...
    # Areas
    emerg = ServiceAreaModel(id=str(uuid.uuid4()), code="EMERG", display_name="Emergencia",
                             load_weight=3, active=True, required_for_daily_coverage=True,
                             created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
    pista = ServiceAreaModel(id=str(uuid.uuid4()), code="PISTA", display_name="Pista",
                             load_weight=2, active=True, required_for_daily_coverage=True,
                             created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
    uci = ServiceAreaModel(id=str(uuid.uuid4()), code="UCI", display_name="UCI",
                           load_weight=4, active=True, required_for_daily_coverage=True,
                           created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
    consul = ServiceAreaModel(id=str(uuid.uuid4()), code="CONSUL", display_name="Consulta Externa",
    ...
    doctor_names = [
        ("Dr. Juan Pérez", "male"), ("Dra. María García", "female"),
        ("Dr. Carlos López", "male"), ("Dra. Ana Martínez", "female"),
        ("Dr. Pedro Ramírez", "male"), ("Dra. Laura Hernández", "female"),
        ("Dr. José Torres", "male"), ("Dr. Miguel Flores", "male"),
        ("Dra. Carmen Díaz", "female"), ("Dr. Roberto Sánchez", "male"),
        ("Dr. Andrés Ruiz", "male"), ("Dra. Patricia Vargas", "female"),
        ("Dr. Fernando Castillo", "male"), ("Dra. Gabriela Mendoza", "female"),
        ("Dr. Luis Ortega", "male"),
    ]
    doctors = []
    ...
    for i, doc in enumerate(doctors):
        ...
        for area in allowed_areas:
            db_session.add(DoctorAllowedAreaModel(
                doctor_id=doc.id, service_area_id=area.id,
            ))
    db_session.flush()
    ...
    for i, doc in enumerate(doctors[:5]):
        db_session.add(CalendarAssignmentModel(
            id=str(uuid.uuid4()),
    ...
    # Unresolved gap
    db_session.add(UnresolvedGapModel(
        id=str(uuid.uuid4()),
        calendar_version_id=cv.id,
    ...
    return {"areas": areas, "ranks": ranks, "departments": depts, "doctors": doctors,
            "calendar": cal, "calendar_version": cv}
...
def _build_llm_responses() -> dict[str, str]:
    ...
    return {
        ...
        "exporta los médicos activos en excel": json.dumps({"action": "export", "query_type": "list_active_doctors", "params": {}, "format": "excel", "confidence": 0.94}),
        "exporta médicos por rango en pdf": json.dumps({"action": "export", "query_type": "count_by_rank", "params": {}, "format": "pdf", "confidence": 0.93}),
        "exporta el ranking de misiones en excel": json.dumps({"action": "export", "query_type": "mission_ranking", "params": {"year": today.year, "month": today.month}, "format": "excel", "confidence": 0.91}),
        "reporte de resumen operativo de este mes en pdf": json.dumps({"action": "export", "query_type": "operational_summary", "params": {"year": today.year, "month": today.month}, "format": "pdf", "confidence": 0.92}),
        "pdf de los huecos sin asignar del mes": json.dumps({"action": "export", "query_type": "unresolved_gaps_month", "params": {"year": today.year, "month": today.month}, "format": "pdf", "confidence": 0.92}),

        # ── Edge cases ────────────────────────────────────────────────
        "asigna a pérez en emergencia mañana": json.dumps({"action": "ambiguous", "query_type": "", "params": {}, "requires_clarification": True, "missing_fields": ["doctor_id"], "response_text": "Encontré más de un médico con ese apellido. ¿Podrías especificar cuál?", "confidence": 0.55}),
        "información confidencial de usuarios del sistema": json.dumps({"action": "reply", "query_type": "", "params": {}, "response_text": "No tengo acceso a esa información.", "confidence": 0.95}),
        "resumen operativo de diciembre 2020": json.dumps({"action": "query", "query_type": "operational_summary", "params": {"year": 2020, "month": 12}, "confidence": 0.92}),
        "médico con nombre que no existe zzz": json.dumps({"action": "query", "query_type": "doctor_detail", "params": {"search": "ZZZNotFound12345", "search_id": "none"}, "confidence": 0.88}),

        # ── Off-template / fallback ───────────────────────────────────
        "doctores que están de vacaciones": json.dumps({"action": "query", "query_type": "", "params": {}, "confidence": 0.75}),
        "médico que tiene más servicios": json.dumps({"action": "query", "query_type": "", "params": {}, "confidence": 0.70}),
        "promedio de servicios": json.dumps({"action": "query", "query_type": "", "params": {}, "confidence": 0.72}),
        "tabla de turnos completa": json.dumps({"action": "query", "query_type": "", "params": {}, "confidence": 0.68}),

        # ── Greetings / conversational ────────────────────────────────
        "qué puedes hacer": json.dumps({"action": "reply", "query_type": "", "params": {}, "response_text": "Puedo ayudarte con información del sistema de turnos médicos: consultar médicos activos, distribución por sexo/rango/departamento, estado de calendarios, asignaciones, historial de servicios, rankings de misiones, huecos sin asignar, y generar reportes en PDF o Excel.", "confidence": 1.0}),
        "gracias": json.dumps({"action": "reply", "query_type": "", "params": {}, "response_text": "¡De nada! Estoy aquí para ayudarte cuando lo necesites.", "confidence": 1.0}),
        "hola": json.dumps({"action": "reply", "query_type": "", "params": {}, "response_text": "¡Hola! Soy el asistente virtual del sistema de gestión de turnos médicos. ¿En qué puedo ayudarte hoy?", "confidence": 1.0}),

        # ── Template queries (most specific first) ────────────────────
        "cuántos médicos activos hay en total": json.dumps({"action": "query", "query_type": "count_doctors_total", "params": {}, "confidence": 0.95}),
        "asignaciones en emergencia": json.dumps({"action": "query", "query_type": "assignments_by_area", "params": {"area_code": "EMERG", "start_date": "2026-05-01", "end_date": "2026-05-31"}, "confidence": 0.90}),
        "historial del doctor con id": json.dumps({"action": "query", "query_type": "doctor_history_60d", "params": {"doctor_id": "will_be_resolved"}, "confidence": 0.85}),
        "historial de garcía": json.dumps({"action": "query", "query_type": "doctor_history_by_name", "params": {"search": "García"}, "confidence": 0.88}),
        "lista de médicos activos": json.dumps({"action": "query", "query_type": "list_active_doctors", "params": {}, "confidence": 0.95}),
        "lista de sargentos": json.dumps({"action": "query", "query_type": "doctors_by_rank", "params": {"rank": "sargento"}, "confidence": 0.94}),
        "lista de cabos": json.dumps({"action": "query", "query_type": "doctors_by_rank", "params": {"rank": "cabo"}, "confidence": 0.94}),
        "médicos hombres": json.dumps({"action": "query", "query_type": "doctors_by_sex", "params": {"sex": "male"}, "confidence": 0.94}),
        "estado del calendario": json.dumps({"action": "query", "query_type": "calendar_status_month", "params": {"year": today.year, "month": today.month}, "confidence": 0.93}),
        "sin disponibilidad": json.dumps({"action": "query", "query_type": "doctors_pending_availability", "params": {"year": today.year, "month": today.month}, "confidence": 0.92}),
        "resumen operativo": json.dumps({"action": "query", "query_type": "operational_summary", "params": {"year": today.year, "month": today.month}, "confidence": 0.94}),
        "ranking de misiones": json.dumps({"action": "query", "query_type": "mission_ranking", "params": {"year": today.year, "month": today.month}, "confidence": 0.90}),
        "médicos trabajan": json.dumps({"action": "query", "query_type": "doctors_working_date", "params": {"date": today.strftime("%Y-%m-%d")}, "confidence": 0.91}),
        "servicios tuvo": json.dumps({"action": "query", "query_type": "assignment_count_by_date_range", "params": {"start_date": "2026-05-01", "end_date": "2026-05-31"}, "confidence": 0.90}),
        "cuántos sargentos": json.dumps({"action": "query", "query_type": "count_by_specific_rank", "params": {"rank": "sargento"}, "confidence": 0.93}),
        "cuántos cabos": json.dumps({"action": "query", "query_type": "count_by_specific_rank", "params": {"rank": "cabo"}, "confidence": 0.93}),
        "cuántos hombres": json.dumps({"action": "query", "query_type": "count_by_specific_sex", "params": {"sex": "male"}, "confidence": 0.93}),
        "cuántas mujeres": json.dumps({"action": "query", "query_type": "count_by_specific_sex", "params": {"sex": "female"}, "confidence": 0.93}),
        "por departamento": json.dumps({"action": "query", "query_type": "count_doctors_by_department", "params": {}, "confidence": 0.95}),
        "por rango": json.dumps({"action": "query", "query_type": "count_by_rank", "params": {}, "confidence": 0.95}),
        "por sexo": json.dumps({"action": "query", "query_type": "count_by_sex", "params": {}, "confidence": 0.95}),
        "huecos sin": json.dumps({"action": "query", "query_type": "unresolved_gaps_month", "params": {"year": today.year, "month": today.month}, "confidence": 0.92}),
        "detalle de": json.dumps({"action": "query", "query_type": "doctor_detail", "params": {"search": "Juan Pérez", "search_id": "none"}, "confidence": 0.90}),
    }
...
class _Sim:
    ...
    def ask(self, user_message: str, category: str, description: str,
        ...
        result = self.agent.process(user_message, telegram_user_id="sim-user-001",
        ...
        for check, expected in expectations.items():
            if check == "response_contains":
                if expected.lower() not in result.response_text.lower():
                    self._fail_reason = f"Expected '{expected}' in response, got: {result.response_text[:100]}"
                    return "FAIL"
                ...
                if bool(result.document_bytes) != expected:
                    self._fail_reason = f"document_bytes expected={expected}, got={bool(result.document_bytes)}"
                    return "FAIL"
            elif check == "document_type":
                if not result.document_filename or not result.document_filename.endswith(expected):
                    self._fail_reason = f"Expected .{expected} file, got: {result.document_filename}"
                    return "FAIL"
            elif check == "action":
    ...
    def report(self) -> None:
        ...
        failed = total - passed
        print(f"\n{'='*70}")
        print(f"  REPORTE DE SIMULACIÓN — AGENTE CONVERSACIONAL TELEGRAM")
        print(f"{'='*70}")
...
def test_full_user_simulation(simulation_env):
    ...
    db_session = simulation_env["db_session"]
    today = date.today()
    month_name = {1:"enero",2:"febrero",3:"marzo",4:"abril",5:"mayo",6:"junio",
                  7:"julio",8:"agosto",9:"septiembre",10:"octubre",11:"noviembre",12:"diciembre"}[today.month]
    ...
    sim.ask("¿cuántos médicos activos hay en total?", "template",
            "1. count_doctors_total", {"response_contains": "Resultado", "action": "query"})

    sim.ask("¿cuántos hombres hay en el servicio?", "template",
            "2. count_by_specific_sex (male)", {"response_contains": "Resultado", "action": "query"})

    sim.ask("¿cuántas mujeres hay en el servicio?", "template",
            "3. count_by_specific_sex (female)", {"response_contains": "Resultado", "action": "query"})

    sim.ask("¿cómo están distribuidos los médicos por sexo?", "template",
            "4. count_by_sex", {"response_contains": "resultados", "action": "query"})

    sim.ask("dame la lista de médicos hombres", "template",
            "5. doctors_by_sex", {"response_contains": "resultados", "action": "query"})

    sim.ask("¿cuántos médicos hay por rango?", "template",
            "6. count_by_rank", {"response_contains": "resultados", "action": "query"})

    sim.ask("¿cuántos cabos hay en el sistema?", "template",
            "7. count_by_specific_rank (cabo)", {"response_contains": "Resultado", "action": "query"})

    sim.ask("¿cuántos sargentos hay?", "template",
            "8. count_by_specific_rank (sargento)", {"response_contains": "Resultado", "action": "query"})

    sim.ask("dame la lista de cabos", "template",
            "9. doctors_by_rank (cabo)", {"response_contains": "resultados", "action": "query"})

    sim.ask("dame la lista de sargentos", "template",
            "10. doctors_by_rank (sargento)", {"response_contains": "resultados", "action": "query"})

    sim.ask("muéstrame la lista de médicos activos", "template",
            "11. list_active_doctors", {"response_contains": "resultados", "action": "query"})

    sim.ask("dame el detalle de Juan Pérez", "template",
            "12. doctor_detail", {"response_contains": "Dr. Juan Pérez", "action": "query"})

    sim.ask(f"¿qué médicos están sin disponibilidad en {month_name}?", "template",
            "13. doctors_pending_availability", {"action": "query"})

    sim.ask(f"¿cuál es el estado del calendario de {month_name}?", "template",
            "14. calendar_status_month", {"response_contains": "Resultado", "action": "query"})

    sim.ask(f"¿qué médicos trabajan hoy {today.strftime('%Y-%m-%d')}?", "template",
            "15. doctors_working_date", {"response_contains": "resultados", "action": "query"})

    sim.ask("¿cuántos servicios tuvo cada médico en el rango 2026-05-01 a 2026-05-31?", "template",
            "16. assignment_count_by_date_range", {"response_contains": "resultados", "action": "query"})
    ...
    sim.ask(f"dame el resumen operativo de {month_name}", "template",
            "18. operational_summary (4 indicadores)", {"response_contains": "Resultado", "action": "query"})
    ...
    sim.ask("¿cuántos médicos hay por departamento?", "template",
            "20. count_doctors_by_department", {"response_contains": "resultados", "action": "query"})
    ...
    sim.ask("¿qué asignaciones en emergencia hay este mes?", "template",
            "22. assignments_by_area", {"response_contains": "Dr. Juan Pérez", "action": "query"})
    ...
    sim.ask("hola", "conversational",
            "28. Saludo", {"response_contains": "Hola", "action": "reply"})

    sim.ask("gracias por la ayuda", "conversational",
            "29. Agradecimiento", {"response_contains": "nada", "action": "reply"})

    sim.ask("¿qué puedes hacer?", "conversational",
            "30. Capacidades", {"response_contains": "turnos", "action": "reply"})
    ...
    sim.ask(f"dame el reporte de resumen operativo de este mes en pdf", "export",
    ...
    sim.ask(f"genera un pdf de los huecos sin asignar del mes", "export",
    ...
    # Return results for programmatic assertion if desired
    return sim.results
